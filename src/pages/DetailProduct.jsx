...
function DetailProduct() {
  ...
  const { data: item, isLoading } = useGetProductQuery({
    idProduct: match.id,
  ...
  if (item) {
    currentPrice =
      item && item.price
        ? Math.round(item.price * ((100 - item.saleOff) / 100))
        : "";
    currentPriceFormat =
  ...
  const onAddToCart = () => {
    toast.success("Bạn vừa thêm sản phẩm vào giỏ hàng thành công!", {
      position: "top-right",
      autoClose: 5000,
    ...
    dispatch(cartAdded({ ...item, quantity }));
  };
              ...
              <div className="col l-7 c-12 m-7">
                <div className="product-right">
                  ...
                  <div className="product-action">
                    <button
                      className="btn product-action-btn product-action-add"
                      onClick={onAddToCart}
                    >
                    ...
                    <Link to="/cart">
                      <button
                        className="btn product-action-btn product-action-buy"
                        onClick={onAddToCart}
                      >
                        Mua Ngay
                      </button>
