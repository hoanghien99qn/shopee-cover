...
function AddForm(props) {
  const { item, collectionArr, onCloseModal } = props;
  const [isLoadingComponent, setIsLoadingComponent] = useState(false);
  let collectionArrIsReady = collectionArr.length > 0;
  ...
  const onSubmit = async (product) => {
    if (!isLoading) {
      try {
        let collectionId =
          document.getElementsByClassName("select-collection")[0].value;
        setIsLoadingComponent(true);
        await editProduct({ product, collectionId });
    ...
    <div className="add-form">
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="form-group">
          ...
          <p className="error-valid">
            {errors.saleOff?.type === "min" || errors.saleOff?.type === "max"
              ? errors.saleOff?.message
              : errors.saleOff?.message
              ? "Sale off is required"
              : ""}
          </p>
        </div>
        <div className="form-group">
        ...
        <div className="form-group">
          <div className="form-group-field">
            ...
            <input
              ...
              type="number"
              id="vote"
              placeholder="VD: 5 ( only integer number )"
            />
          </div>
          <p className="error-valid">
            {errors.vote?.type === "min" || errors.vote?.type === "max"
              ? errors.vote?.message
              : errors.vote?.message
              ? "Vote is required"
              : ""}
